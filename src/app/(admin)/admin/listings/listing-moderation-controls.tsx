...
import { deletePropertyListing, moderatePropertyListing } from './actions';
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      <Button
        ...
        type="button"
        variant="ghost"
        className={`${dashboardButtonRadiusClass} text-destructive hover:text-destructive`}
        disabled={pending}
        onClick={() =>
          start(() => {
            void (async () => {
              const res = await deletePropertyListing(listingId);
              setNotice(res && 'error' in res ? res.error ?? 'Failed.' : 'Listing deleted.');
            })();
          })
        }
      >
