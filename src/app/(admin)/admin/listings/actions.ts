...
export async function saveProperty(prevState: PropertySaveState, formData: FormData): Promise<PropertySaveState> {
  ...
  if (!supabase) {
    return { error: 'Configuration missing. Please contact support.' };
  }
  ...
  if (role === 'agent') {
    if (!viewer.onboardingPaid) {
      return { error: 'Complete your NGN 5,000 onboarding payment before creating listings.' };
    }
  ...
  revalidateTag('active-properties');
...
export async function deletePropertyListing(listingId: string) {
  const viewer = await getViewerContext();
  if (!viewer) return { error: 'Not signed in.' };
  if (viewer.role !== 'admin') return { error: 'Only admins can delete listings.' };

  const supabase = await createClient();
  if (!supabase) return { error: 'Configuration missing.' };
  const { data: listing } = await supabase.from('properties').select('title, agent_id').eq('id', listingId).maybeSingle();
  ...
  if (listing?.agent_id) {
    await createNotification({
      ...
      body: `${listing.title} was removed by admin.`,
