...
export type SignUpState =
  | { error: string }
  | { success: true; needsEmailConfirmation: boolean; role: 'client' | 'agent'; email: string }
  | null;
...
export async function signUp(prevState: SignUpState, formData: FormData): Promise<SignUpState> {
  ...
  if (role === 'agent' && confirmPassword && password !== confirmPassword) {
    return { error: "Password doesn't match" };
  }
  if (role === 'agent' && !confirmPassword) {
  ...
  if (role === 'agent') {
    ...
    if (serviceInterest === 'travel' || serviceInterest === 'construction') {
      if (!phone) return { error: 'Phone number is required for this client account.' };
    }
  ...
  const callbackNext = role === 'agent' ? '/agent/onboarding' : '/dashboard';
  ...
  if (!needsEmailConfirmation) {
    redirect(role === 'agent' ? '/agent/onboarding' : '/dashboard');
  }

  return { success: true, needsEmailConfirmation, role, email };
}
